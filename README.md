...
在线页面不是一张结果截图，而是一套可切换的研究工作台：完整展示研究总览、消息评分、趋势流畅度、策略研究室、自动研究 Loop、支撑压力、趋势因子组合、Gamma 趋势、30/60 周期实验、模拟盘和系统边界。每个页面同时给出界面、要解决的问题、设计理念和实现要点。页面读取仓库构建时生成的合成数据，不连接交易账户、不调用行情接口。希望检查 FastAPI 接口或运行测试时，再按下方步骤克隆项目。
...
这个公开版把上述问题组织为 11 个可以独立查看、又能串成研究闭环的界面模块。
